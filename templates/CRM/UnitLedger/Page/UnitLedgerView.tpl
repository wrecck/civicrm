{* Standalone Unit Ledger Page *}
<div class="crm-container">
	<div class="crm-section">
		<h2>{ts}Unit Ledger{/ts}</h2>

		{* Simple filter summary *}
		<div class="messages status">
			{if $caseId}{ts}Case:{/ts} {$caseId} {/if}
			{if $contactId}&nbsp; {ts}Contact:{/ts} {$contactId} {/if}
			{if $program}&nbsp; {ts}Program:{/ts} {$program} {/if}
			{if $entryType}&nbsp; {ts}Entry Type:{/ts} {$entryType} {/if}
		</div>
